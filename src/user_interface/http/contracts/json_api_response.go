...
import (
	sharedAppError "credens/src/shared/application/error"
	sharedDomainError "credens/src/shared/domain/error"
	sharedInfraError "credens/src/shared/infrastructure/error"
	"fmt"
)
...
func NewJSONAPIErrorObject(err error, httpStatus int) *JSONAPIErrorObject {
	...
	if domainError, ok := err.(sharedDomainError.DomainError); ok {
	...
	if appError, ok := err.(sharedAppError.ApplicationError); ok {
		return &JSONAPIErrorObject{
			HttpStatus: httpStatus,
			Code:   appError.Code,
	...
	if infraError, ok := err.(sharedInfraError.InfrastructureError); ok {
		return &JSONAPIErrorObject{
			HttpStatus: httpStatus,
